...
//        guard let original = UIImage(contentsOfFile: path) else { return }
        guard let pic = UIImage(uncached: image) else { return }
        
		let renderer = UIGraphicsImageRenderer(size: pic.size)

		let rounded = renderer.image { ctx in
			ctx.cgContext.addEllipse(in: CGRect(origin: CGPoint.zero, size: pic.size))
