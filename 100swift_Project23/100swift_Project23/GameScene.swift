...
class GameScene: SKScene {
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for case let node as SKSpriteNode in nodesAtPoint {
            if node.name == "enemy" {
                ...
                let scaleOut = SKAction.scale(to: 0.015, duration: 0.5)
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        if enemyType == 0 {
            ...
            if let path = Bundle.main.url(forResource: "sliceBombFuce", withExtension: "caf") {
            ...
            enemy = SKSpriteNode(imageNamed: "specPenguin")
            run(SKAction.playSoundFileNamed("launch.caf", waitForCompletion: false))
            enemy.name = "specEnemy"
        ...
        if enemy.name == "specEnemy" {
            enemy.physicsBody?.velocity = CGVector(dx: randomXVelocity * 70, dy: randomYVelocity * 70)
        }
        ...
        if activeEnemies.count > 0 {
            for (index, node) in activeEnemies.enumerated().reversed() {
                if node.position.y < -140 {
                    ...
                    if node.name == "enemy" || node.name == "specEnemy" {
                        node.name = ""
                        subtractLife()
