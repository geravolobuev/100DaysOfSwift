...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createWind() {
        let wind = Int.random(in: -5...5)
        physicsWorld.gravity = CGVector(dx: wind, dy: -10)
        if wind > 0 {
            viewController?.windLabel.text = "WIND >>>"
        } else if wind < 0 {
            viewController?.windLabel.text = "<<< WIND"
        } else {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if firstNode.name == "banana" && secondNode.name == "player1" {
            ...
            viewController?.player2ScoreNumber += 1
        }
        
        if firstNode.name == "banana" && secondNode.name == "player2" {
            destroy(player: player2)
            viewController?.player1ScoreNumber += 1
        }
    }
    ...
    func destroy(player: SKSpriteNode) {
        ...
        if viewController!.player1ScoreNumber > 2 || viewController!.player2ScoreNumber > 2 {
